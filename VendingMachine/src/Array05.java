public class Array05 {
    public static void main(String[] args) {
        ItemForArray[] array1 = new ItemForArray[3];
        array1[0] = new ItemForArray(500, "사과");
        array1[1] = new ItemForArray(300, "바나나");
        array1[2] = new ItemForArray(700, "수박");

        ItemForArray[] array2 = new ItemForArray[]{
                new ItemForArray(500, "사과"),
                new ItemForArray(300, "바나나"),
                new ItemForArray(700, "수박")
        };
        ItemForArray[] array3 = {
                new ItemForArray(500, "사과"),
                new ItemForArray(300, "바나나"),
                new ItemForArray(700, "수박")
        };

        System.out.print(array1[0].getName());
        System.out.println(array1[0].getPrice());
        System.out.print(array1[1].getName());
        System.out.println(array1[1].getPrice());
        System.out.print(array1[2].getName());
        System.out.println(array1[2].getPrice());


        System.out.print(array2[0].getName());
        System.out.println(array2[0].getPrice());
        System.out.print(array2[1].getName());
        System.out.println(array2[1].getPrice());
        System.out.print(array2[2].getName());
        System.out.println(array2[2].getPrice());

        System.out.print(array3[0].getName());
        System.out.println(array3[0].getPrice());
        System.out.print(array3[1].getName());
        System.out.println(array3[1].getPrice());
        System.out.print(array3[2].getName());
        System.out.println(array3[2].getPrice());

    }
}
